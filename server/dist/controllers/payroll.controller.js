...
const downloadPayslipPDF = async (req, res) => {
    try {
        ...
        if (Number(item.allowances))
            drawRow('Allowances', Number(item.allowances));
        doc.moveTo(50, y).lineTo(545, y).strokeColor('#e2e8f0').stroke();
        y += 4;
        drawRow('GROSS PAY', Number(item.grossPay), '#1e293b', true);
        y += 10;
        doc.fontSize(11).font('Helvetica-Bold').fillColor('#ef4444').text('DEDUCTIONS', 50, y);
        y += 20;
        drawRow('Income Tax (PAYE)', Number(item.tax), '#ef4444');
        if (Number(item.ssnit))
            drawRow(item.currency === 'GNF' ? 'CNSS (2.5%)' : 'SSNIT (5.5%)', Number(item.ssnit), '#ef4444');
        if (Number(item.otherDeductions))
            drawRow('Other Deductions', Number(item.otherDeductions), '#ef4444');
        doc.moveTo(50, y).lineTo(545, y).strokeColor('#e2e8f0').stroke();
        y += 4;
        ...
        // ── Footer ───────────────────────────────────────────────
        doc.fontSize(8).fillColor('#94a3b8').text(`Generated on ${new Date().toLocaleDateString()} · ${companyName} · Confidential`, 50, 780, { align: 'center', width: 495 });
        doc.end();
    }
    catch (err) {
