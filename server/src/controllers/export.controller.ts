...
export const exportEmployeesPDF = async (req: Request, res: Response) => {
  try {
    ...
    const { brandColor, companyName } = await drawBrandedHeader(pdfDoc, orgId, i18n.translate('pdf.employee_directory.title', lang), lang);

    pdfDoc.fontSize(11).font('Helvetica').fillColor('#64748b').text(`${i18n.translate('pdf.common.generated', lang)}: ${new Date().toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US')} · ${employees.length} ${i18n.translate('pdf.employee_directory.active_count', lang)}`, 50, 140);
    ...
    pdfDoc.text(i18n.translate('pdf.employee_directory.cols.name', lang), 60, y + 6);
    pdfDoc.text(i18n.translate('pdf.employee_directory.cols.role', lang), 220, y + 6);
    pdfDoc.text(i18n.translate('pdf.employee_directory.cols.dept', lang), 365, y + 6);
    pdfDoc.text(i18n.translate('pdf.employee_directory.cols.status', lang), 470, y + 6);
    y += 25;
    ...
    employees.forEach((emp, i) => {
      ...
      const isEven = i % 2 === 0;
      if (isEven) pdfDoc.rect(50, y - 3, 495, 22).fillColor('#f8fafc').fill();
...
export const exportEmployeeDossierPDF = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const orgId = (req as any).user?.organizationId || 'default-tenant';
    ...
    const drawSectionCard = (title: string, fields: { label: string; value: string | null | undefined; fullWidth?: boolean }[], isContinued = false) => {
    ...
    const safeDrawSection = (title: string, fields: any[]) => {
        try {
            drawSectionCard(title, fields);
        } catch (e: any) {
            if (e.type !== 'PAGE_BREAK_RECURSION') throw e;
        }
    };
    ...
    // 1. Personal Identity
    safeDrawSection(i18n.translate('pdf.dossier.personal_info', lang), [
      { label: i18n.translate('pdf.dossier.email', lang), value: employee.email },
      { label: i18n.translate('pdf.dossier.phone', lang), value: employee.contactNumber },
      { label: i18n.translate('pdf.dossier.address', lang), value: employee.address, fullWidth: true },
      { label: i18n.translate('pdf.dossier.nationality', lang), value: employee.nationality },
      { label: i18n.translate('pdf.dossier.dob', lang), value: employee.dob ? format(new Date(employee.dob), 'PP') : '—' },
      { label: i18n.translate('pdf.dossier.gender', lang), value: employee.gender },
      { label: i18n.translate('pdf.dossier.marital_status', lang) || 'Marital Status', value: employee.maritalStatus },
    ...
    safeDrawSection(i18n.translate('pdf.dossier.employment_details', lang), [
      { label: i18n.translate('pdf.dossier.status', lang), value: employee.status },
      { label: i18n.translate('pdf.dossier.role', lang), value: employee.role },
      { label: i18n.translate('pdf.dossier.type', lang), value: employee.employmentType },
      { label: i18n.translate('pdf.dossier.join_date', lang) || 'Join Date', value: employee.joinDate ? format(new Date(employee.joinDate), 'PP') : '—' },
      { label: i18n.translate('pdf.dossier.primary_manager', lang), value: employee.supervisor?.fullName, fullWidth: true },
      { label: i18n.translate('pdf.dossier.matrix_manager', lang), value: functionalManager?.fullName, fullWidth: true },
    ]);

    // 3. Compliance & Financials
    safeDrawSection(i18n.translate('pdf.dossier.compliance', lang) || 'Compliance & Financials', [
    ...
    safeDrawSection(i18n.translate('pdf.dossier.emergency_protocol', lang) || 'Emergency Protocol', [
      { label: i18n.translate('pdf.dossier.contact_name', lang), value: employee.emergencyContactName },
      { label: i18n.translate('pdf.dossier.contact_phone', lang), value: employee.emergencyContactPhone },
...
export const exportLeavePDF = async (req: Request, res: Response) => {
  try {
    ...
    // --- Header (Branded) ---
    const { brandColor, companyName } = await drawBrandedHeader(pdfDoc, orgId, i18n.translate('pdf.leave_request.title', lang), lang);
    ...
    if ((leave as any).handoverNotes) {
      if (y > 700) { pdfDoc.addPage(); y = 50; }
      y = drawSectionHeader(i18n.translate('pdf.leave_request.handover_notes', lang), y);
      pdfDoc.fillColor('#1e293b').font('Helvetica').fontSize(9).text((leave as any).handoverNotes, 50, y, { width: 495, align: 'justify', lineGap: 3 });
      y += pdfDoc.heightOfString((leave as any).handoverNotes, { width: 495 }) + 40;
    }

    // Section 4: Approvals
    if (y > 600) { pdfDoc.addPage(); y = 50; }
    ...
    const drawApprovalColumn = (label: string, status: string, approver: string, date: Date | null, xPosition: number, currentY: number) => {
      ...
      if (date) {
         pdfDoc.fillColor('#94a3b8').fontSize(7).text(format(new Date(date), 'MMM dd, yyyy HH:mm'), xPosition + 15, currentY + 50);
      }
    ...
    const approvalY = y;
    drawApprovalColumn(i18n.translate('pdf.leave_request.lm_approval', lang) || 'Line Manager Approval', s1, leave.manager?.fullName || 'Personnel Lead', null, 50, approvalY);
    drawApprovalColumn(i18n.translate('pdf.leave_request.md_signoff', lang) || 'Executive Sign-off (MD)', s2, leave.hrReviewer?.fullName || 'Managing Director', null, 305, approvalY);
...
// --- BRANDING HELPER ---
const drawBrandedHeader = async (pdfDoc: any, orgId: string, title: string, lang: string = 'en', subtitleOverride?: string) => {
  ...
  pdfDoc.fillColor('#64748b').font('Helvetica').fontSize(8).text(subtitleOverride || i18n.translate('pdf.common.official_record', lang), textX, headerY + 32);
...
export const exportAppraisalPDF = async (req: Request, res: Response) => {
  try {
    ...
    pdfDoc.fillColor(brandColor).font('Helvetica-Bold').fontSize(7).text(i18n.translate('pdf.performance.dossier', lang).toUpperCase(), 65, 175);
    ...
    if (packet.finalScore) {
       ...
       pdfDoc.fontSize(6).text(i18n.translate('pdf.performance.final_rating', lang).toUpperCase(), 450, 220, { width: 70, align: 'center' });
    }

    let y = 280;

    // --- Section Utility ---
    const drawSectionMarker = (label: string, curY: number) => {
    ...
    for (const rev of packet.reviews) {
      if (y > 650) { pdfDoc.addPage(); y = 60; }
      
      const stageLabel = i18n.translate(`appraisal.stages.${rev.reviewStage}`, lang) || rev.reviewStage.replace(/_/g, ' ');
      y = drawSectionMarker(`${stageLabel}: ${rev.reviewer?.fullName || 'SELF'}`, y);

      if (rev.summary) {
        pdfDoc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7).text(i18n.translate('pdf.performance.reviewer_summary', lang).toUpperCase(), 65, y);
    ...
    if (packet.finalVerdict || packet.disputeResolution) {
      if (y > 600) { pdfDoc.addPage(); y = 60; }
      y = drawSectionMarker(i18n.translate('pdf.performance.verdict_conclusion', lang), y);
    ...
    // ─── SIGNATURE BLOCK ───
    if (y > 680) { pdfDoc.addPage(); y = 100; } else { y = 700; }
    
    pdfDoc.moveTo(65, y).lineTo(230, y).strokeColor('#e2e8f0').stroke();
    pdfDoc.fontSize(7).fillColor('#94a3b8').text(i18n.translate('pdf.performance.employee_signature', lang).toUpperCase(), 65, y + 8);
...
export const exportTargetPDF = async (req: Request, res: Response) => {
  try {
    ...
    pdfDoc.fontSize(11).font('Helvetica-Bold').fillColor(brandColor).text(i18n.translate('pdf.roadmap.strategic_roadmap', lang), 50, 160);
    pdfDoc.fontSize(8).font('Helvetica').fillColor('#64748b').text(i18n.translate('pdf.roadmap.alignment_dossier', lang), 50, 175);
    
    pdfDoc.fontSize(14).font('Helvetica-Bold').fillColor('#1e293b').text(employee.fullName, 50, 190);
    pdfDoc.fontSize(10).font('Helvetica').fillColor('#64748b').text(`${employee.jobTitle}  |  ${employee.departmentObj?.name || i18n.translate('pdf.roadmap.unassigned', lang)}`, 50, 210);

    // Summary Table Header
    let tableY = 240;
    pdfDoc.rect(50, tableY, 495, 20).fill(brandColor);
    pdfDoc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(9).text(i18n.translate('pdf.roadmap.target_mission', lang), 60, tableY + 6);
    pdfDoc.text(i18n.translate('pdf.roadmap.due_date', lang), 350, tableY + 6);
    pdfDoc.text(i18n.translate('pdf.roadmap.weight', lang), 420, tableY + 6);
    pdfDoc.text(i18n.translate('pdf.roadmap.status', lang), 480, tableY + 6);
    ...
    for (const target of targets) {
      ...
      // Metrics
      pdfDoc.fillColor('#94a3b8').font('Helvetica-Bold').fontSize(7).text(i18n.translate('pdf.roadmap.kpi_metrics', lang), 65, y);
      y += 15;
