...
const drawBrandedHeader = async (pdfDoc: any, orgId: string, title: string, lang: string = 'en', subtitleOverride?: string) => {
  ...
  pdfDoc.fillColor('#64748b').font('Helvetica').fontSize(10).text(subtitleOverride || i18n.translate('pdf.common.official_record', lang), textX, 88);
...
export const exportAppraisalPDF = async (req: Request, res: Response) => {
  try {
    ...
    pdfDoc.fillColor(brandColor).font('Helvetica-Bold').fontSize(8).text(i18n.translate('pdf.performance.dossier', lang), 65, 175);
    
    pdfDoc.fillColor('#1e293b').fontSize(14).text(packet.employee.fullName, 65, 190).font('Helvetica-Bold');
    pdfDoc.fillColor('#64748b').fontSize(10).font('Helvetica').text(`${packet.employee.jobTitle}  |  ${packet.employee.departmentObj?.name || i18n.translate('pdf.roadmap.unassigned', lang)}`, 65, 210);
    ...
    if (packet.finalScore) {
       ...
       pdfDoc.fontSize(7).text(i18n.translate('pdf.performance.final_rating', lang), 430, 210, { width: 100, align: 'center' });
    }
    ...
    for (const rev of packet.reviews) {
      ...
      if (rev.summary) {
        pdfDoc.fillColor('#334155').font('Helvetica-Bold').fontSize(9).text(`${i18n.translate('pdf.performance.reviewer_summary', lang)}:`, 65, y);
      ...
      try {
        ...
        if (parsed.competencyScores) {
          pdfDoc.fillColor(brandColor).font('Helvetica-Bold').fontSize(8).text(i18n.translate('pdf.performance.competency_breakdown', lang), 65, y);
          y += 15;
    ...
    if (packet.finalVerdict || packet.disputeResolution) {
      ...
      y += 15;
      pdfDoc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(11).text(i18n.translate('pdf.performance.verdict_conclusion', lang), 50, y);
      y += 20;
    ...
    // Signatures
    if (y > 700) { pdfDoc.addPage(); y = 100; } else { y = 720; }
    pdfDoc.moveTo(50, y).lineTo(200, y).strokeColor(brandColor).stroke();
    pdfDoc.fontSize(8).fillColor('#64748b').text(i18n.translate('pdf.common.signature', lang), 50, y + 5);
    
    pdfDoc.moveTo(395, y).lineTo(545, y).strokeColor(brandColor).stroke();
    pdfDoc.text(i18n.translate('pdf.common.authority', lang), 395, y + 5);
