...
export const paystackWebhook = async (req: Request, res: Response) => {
  try {
    ...
    if (event.event === 'charge.success') {
      ...
      // Update organization branding/status if multi-tenant
      const user = await prisma.user.findFirst({ where: { id: userId } });
      if (user?.organizationId) {
        await prisma.organization.update({
          where: { id: user.organizationId },
          data: { subscriptionPlan: 'PRO', billingStatus: 'ACTIVE' }
        });
      }
...
export const getSubscriptionStatus = async (req: Request, res: Response) => {
  const org = await prisma.organization.findFirst({
    where: { id: req.user?.organizationId || 'default-tenant' },
    include: { settings: true }
  });

  const sub = await prisma.subscription.findFirst({
    where: { organizationId: req.user?.organizationId || 'default-tenant' },
    orderBy: { createdAt: 'desc' },
    select: { plan: true, status: true, currentPeriodEnd: true, paystackRef: true, createdAt: true }
  });
  ...
  res.json({
    ...
    monthlyPrice: org?.settings?.monthlyPriceGHS,
    annualPrice: org?.settings?.annualPriceGHS,
    paystackConfigured: !!org?.settings?.paystackPublicKey,
    paystackPublicKey: org?.settings?.paystackPublicKey || null,
