...
22. Three architecturally separate "cycle" models exist in `schema.prisma`: `Cycle` (legacy, `name`/`type`, no relations), `ReviewCycle` (`title`, relates to `PerformanceReviewV2`), and `AppraisalCycle` (`title`/`period`, relates to `AppraisalPacket` — the one that actually matters for the main Appraisal flow this session worked on). **Correction, caught by an automated review on the PR that introduced this note**: `ReviewCycle` is *not* dead code — `performance-v2.controller.ts::createReview` queries it, and `enterprise.controller.ts::upsertPerformanceReview` selects the active one. Do not delete it as a "safe" cleanup without first migrating those two consumers. This is why bug #17 existed — three overlapping cycle concepts is still worth a real consolidation pass, just not a delete-on-sight one.
...
6. **The `ReviewCycle` model (§2.22).** Not dead — confirmed live in `performance-v2.controller.ts`/`enterprise.controller.ts`. A real consolidation pass (migrate those two consumers onto `AppraisalCycle`, then remove it) is still worth doing — three overlapping "cycle" concepts is exactly the kind of thing that causes another bug like #17 down the line — just don't approach it as a delete-on-sight cleanup.
