  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
      className={cn(
        ...
        getStatusConfig(t)[target.status]?.ring
      )}>
                ...
                <div className="flex flex-wrap items-center gap-3 shrink-0">
                  ...
                  {canEdit && (
                    <div className="flex gap-2 p-1.5 bg-white rounded-2xl border border-slate-200 shadow-sm">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); if(window.confirm(t('targets.delete_confirm'))) onDelete?.(); }} className="p-2.5 rounded-xl text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  )}
