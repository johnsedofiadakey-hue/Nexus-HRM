          ...
          <div className="flex items-center justify-center">
             <div className="grid grid-cols-3 gap-4 w-full max-w-2xl">
               {[
                 ...
                 <div key={idx} className="flex flex-col items-center gap-3 relative">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border-2 transition-all ${idx === 1 ? 'bg-[var(--primary)] border-[var(--primary)] text-white shadow-lg shadow-[var(--primary)]/20' : 'bg-[var(--primary)]/20 border-[var(--primary)]/30 text-[var(--primary)]'}`}>
                      <step.icon size={20} />
                    </div>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${idx === 1 ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)]'}`}>{step.label}</span>
                    {idx < 2 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8">
          <motion.div 
            ...
            className="p-8 h-full rounded-2xl bg-[var(--warning)]/5 border border-[var(--warning)]/10 flex items-center gap-6"
          >
            <div className="w-14 h-14 rounded-2xl bg-[var(--warning)]/10 text-[var(--warning)] flex items-center justify-center border border-[var(--warning)]/20">
              <AlertCircle size={24} />
            </div>
            <div>
              <p className="text-[11px] font-black text-[var(--text-primary)] uppercase tracking-[0.2em] mb-1">{t('manager_dashboard.active_guidance')}</p>
              <p className="text-xs font-medium text-[var(--warning)]/80 uppercase tracking-widest leading-relaxed">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} className="nx-card p-10 border-[var(--primary)]/20">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link to="/reviews/team" className="text-[11px] font-black uppercase tracking-widest text-[var(--primary)] hover:text-[var(--primary)]/80 transition-colors flex items-center gap-2 no-underline">
          ...
          <div className="space-y-4">
             {stats.pendingReviews > 0 ? (
                <div className="p-8 rounded-2xl bg-[var(--primary)]/5 border border-[var(--primary)]/10 flex flex-col sm:flex-row items-center justify-between gap-6 group hover:bg-[var(--primary)]/10 transition-all">
                   ...
                   <Link to="/reviews/team" className="px-8 py-3 rounded-xl bg-[var(--primary)] text-[var(--text-inverse)] text-[11px] font-black uppercase tracking-widest shadow-lg shadow-[var(--primary)]/40 hover:scale-105 transition-all no-underline">{t('manager_dashboard.review_now')}</Link>
                </div>
             ) : (
        ...
        <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} className="nx-card p-10 border-[var(--primary)]/20">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link to="/kpi/team" className="text-[11px] font-black uppercase tracking-widest text-[var(--primary)] hover:opacity-80 transition-colors flex items-center gap-2 no-underline">
        ...
        {/* TEAM PERFORMANCE (Primary Track) */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="xl:col-span-2 nx-card p-10 border-[var(--primary)]/20 bg-[var(--primary)]/[0.02]">
          <div className="flex items-center justify-between mb-10">
            ...
            <div className="flex items-center gap-3 text-3xl font-black text-[var(--success)]">
          ...
          <div className="h-3 w-full bg-[var(--bg-elevated)] rounded-full overflow-hidden mb-12">
             <motion.div 
               ...
               className="h-full bg-gradient-to-r from-[var(--primary)] to-[var(--accent)] shadow-[0_0_20px_rgba(var(--primary-rgb),0.3)]"
             />
          </div>
