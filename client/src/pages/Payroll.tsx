...
const Payroll = () => {
  ...
  const handleApprove = async (runId: string) => {
    ...
    } catch (err: any) { toast.info(String(err?.response?.data?.error || 'Authorization protocol failure')); }
  };
  ...
  const saveItem = async () => {
    ...
    } catch (err: any) { toast.info(String(err?.response?.data?.error || 'Data integrity failure')); }
                        ...
                        <div className="overflow-x-auto custom-scrollbar">
                          <table className="nx-table">
                            ...
                            <tbody className="divide-y divide-white/[0.02]">
                              {(Array.isArray(selectedRun?.items) ? selectedRun.items : []).map((item: any) => {
